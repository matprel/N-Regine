...
class Model:
    ...
    def _ricorsione(self, parziale, N):
        self.N_iterazioni += 1 #conto quante volte entro nella ricorsione
        #condizione terminale
        if len(parziale) == N:
            # print(parziale)
            if self._is_soluzione_nuova(parziale): #aggiungo la soluzione solo se è nuova
                self.N_soluzioni += 1 #conto quante soluzioni ho trovato
                self.soluzioni.append(copy.deepcopy(parziale))
        #caso ricorsivo: doppio ciclo for, uno per le righe e uno per le colonne
        else:
            for row in range(N):
                for col in range(N):
                    parziale.append((row, col)) #tupla
                    if self._regina_ammissibile(parziale):
                        self._ricorsione(parziale, N)
                    parziale.pop() #rimuove l'ultimo elemento
    ...
    def _regina_ammissibile(self, parziale):
        ...
        for regina in parziale[:len(parziale)-1]:
            # controllare righe
                if ultima_regina[0] == regina[0]: #perchè è una tupla (riga, colonna)
                    return False
            # controllare colonne
                if ultima_regina[1] == regina[1]:
                    return False
            # controllare diagonali: se la differenza o la somma degli indici rimane costante, sono sulla stessa diagonale
                # check row-col
